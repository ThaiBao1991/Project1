...
class StampApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Công cụ đóng dấu PDF - Template Matching")
    ...
    def bind_canvas_for_region_select(self):
        """Bind các event cho chế độ khoanh vùng"""
        self.canvas.bind("<ButtonPress-1>", self.on_mouse_down)
        self.canvas.bind("<B1-Motion>", self.on_mouse_move)
        self.canvas.bind("<ButtonRelease-1>", self.on_mouse_up)

    def unbind_canvas_for_region_select(self):
        """Unbind các event khi chuyển sang chế độ khác"""
        self.canvas.unbind("<ButtonPress-1>")
        self.canvas.unbind("<B1-Motion>")
        self.canvas.unbind("<ButtonRelease-1>")

    def resize_stamp_image_high_quality(self, img, target_width, target_height):
        """
        Resize ảnh với chất lượng cao, chống vỡ nét
        Sử dụng kỹ thuật multi-step resize + INTER_AREA cho thu nhỏ
        """
        try:
            # Chuyển PIL sang OpenCV
            img_cv = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
            
            current_h, current_w = img_cv.shape[:2]
            target_w, target_h = target_width, target_height
            
            # Nếu là thu nhỏ (downscale)
            if target_w < current_w and target_h < current_h:
                # Sử dụng INTER_AREA - tốt nhất cho thu nhỏ
                resized_cv = cv2.resize(img_cv, (target_w, target_h), interpolation=cv2.INTER_AREA)
            else:
                # Nếu là phóng to, resize nhiều bước để tránh vỡ nét
                steps = []
                step_w = current_w
                step_h = current_h
                
                # Tính số bước cần thiết (tối đa 5 bước)
                while step_w < target_w and step_h < target_h:
                    step_w = min(step_w * 1.5, target_w)
                    step_h = min(step_h * 1.5, target_h)
                    steps.append((int(step_w), int(step_h)))
                
                # Thực hiện resize nhiều bước
                current_img = img_cv
                for w, h in steps:
                    current_img = cv2.resize(current_img, (w, h), interpolation=cv2.INTER_LANCZOS4)
                
                resized_cv = current_img
            
            # Chuyển OpenCV về PIL
            resized_rgb = cv2.cvtColor(resized_cv, cv2.COLOR_BGR2RGB)
            return Image.fromarray(resized_rgb)
        except Exception as e:
            self.log(f"Lỗi resize ảnh chất lượng cao: {e}")
            # Fallback về phương pháp cũ
            return img.resize((target_width, target_height), Image.Resampling.LANCZOS)

    def activate_center_picker(self):
        """Kích hoạt chế độ chọn tâm - tách biệt hoàn toàn với khoanh vùng"""
        self.picking_center = True
        self.center_label.config(text="Đang chọn tâm... Click vào vùng đã khoanh", foreground="orange")
        self.canvas.config(cursor="target")
        
        # Unbind các event khoanh vùng
    ...
    def pick_center(self, event):
        """Chọn tâm con dấu - không ảnh hưởng đến vùng khoanh"""
        if self.picking_center:
            if self.stamp_bbox:
                x, y = event.x, event.y
                # Kiểm tra click có trong vùng khoanh không
            ...
            # Thoát chế độ chọn tâm
            self.picking_center = False
            self.canvas.config(cursor="cross")
            self.canvas.unbind("<Button-1>")
            
            # Khôi phục các event khoanh vùng
            self.bind_canvas_for_region_select()
    ...
    def on_mouse_down(self, event):
        if not self.picking_center:  # Chỉ xử lý khi không ở chế độ chọn tâm
    ...
    def create_template_from_bbox(self):
        """Tạo ảnh mẫu từ vùng đã khoanh để dùng cho template matching"""
        ...
        try:
            ...
            self.template_image = original_img.crop((x1, y1, x2, y2))
            self.log(f"Đã tạo ảnh mẫu kích thước: {self.template_image.size}")
            return True
        except Exception as e:
        ...
        try:
            if self.temp_cropped_image and os.path.exists(self.temp_cropped_image):
                ...
                if new_width > 0 and new_height > 0:
                    # Sử dụng resize chất lượng cao
                    stamp_img = self.resize_stamp_image_high_quality(stamp_img, new_width, new_height)
                ...
                if self.stamp_center:
                    ...
                    if stamp_img.mode == 'RGBA':
                        result_img.paste(stamp_img, (paste_x, paste_y), stamp_img)
                    else:
                        result_img.paste(stamp_img, (paste_x, paste_y))
    ...
    def find_best_match_position(self, page_image):
        """Tìm vị trí khớp nhất của template trên ảnh trang"""
        if not self.template_image or not self.use_template_matching.get():
            return None
        
        try:
            template_gray = cv2.cvtColor(np.array(self.template_image), cv2.COLOR_RGB2GRAY)
    ...
    def add_stamp_to_pdf(self, input_pdf, output_pdf, stamp_path, scale_percent, position_center=None):
        """Chèn ảnh con dấu vào PDF - hỗ trợ template matching nếu bật"""
        try:
            temp_output = None
            ...
            if new_width > 0 and new_height > 0:
                # Sử dụng resize chất lượng cao
                img = self.resize_stamp_image_high_quality(img, new_width, new_height)
            else:
                self.log(f"Cảnh báo: Kích thước ảnh quá nhỏ ({new_width}x{new_height})")
                return False
            
            temp_img_path = os.path.join(self.script_dir, "temp_stamp.png")
            img.save(temp_img_path, "PNG", optimize=True)
            
            if self.use_template_matching.get() and self.template_image:
                for page_num in range(len(doc)):
                    ...
                    if match_pos:
                        ...
                        img_rect = fitz.Rect(
                            center_x_pdf - new_width/2,
                            center_y_pdf - new_height/2,
                            center_x_pdf + new_width/2,
                            center_y_pdf + new_height/2
                        )
                        page.insert_image(img_rect, filename=temp_img_path)
                        self.log(f"Trang {page_num+1}: Đã đóng dấu tại vị trí tìm được")
                    else:
                ...
                for page_num in range(len(doc)):
                    page = doc[page_num]
                    img_rect = fitz.Rect(
                        position_center[0] - new_width/2,
                        position_center[1] - new_height/2,
                        position_center[0] + new_width/2,
                        position_center[1] + new_height/2
                    )
                    page.insert_image(img_rect, filename=temp_img_path)
                    self.log(f"Trang {page_num+1}: Đã đóng dấu tại tọa độ cố định")
    ...
    def test_stamp(self):
        ...
        if self.use_template_matching.get() and not self.template_image:
        ...
        if self.add_stamp_to_pdf(self.pdf_sample_path.get(), output_path,
            ...
            self.log(f"Test thành công! Đã xử lý {self.total_pages} trang")
    ...
    def start_stamp_all(self):
        ...
        for pdf_file in pdf_files:
            ...
            self.log(f"Đang xử lý: {pdf_file} ({page_count} trang)")

...
class StampApp:
    ...
    def resize_stamp_image_high_quality(self, img, target_width, target_height):
        try:
            ...
            if target_width < current_w and target_height < current_h:
                ...
                temp_img.thumbnail((target_width, target_height), Image.Resampling.LANCZOS)
                if temp_img.size != (target_width, target_height):
                    temp_img = temp_img.resize((target_width, target_height), Image.Resampling.LANCZOS)
                ...
                while step_w < target_width and step_h < target_height:
                    step_w = min(int(step_w * 1.5), target_width)
                    step_h = min(int(step_h * 1.5), target_height)
                    steps.append((step_w, step_h))
                
                for w, h in steps:
                    current_img = current_img.resize((w, h), Image.Resampling.LANCZOS)
                
                if current_img.size != (target_width, target_height):
                    current_img = current_img.resize((target_width, target_height), Image.Resampling.LANCZOS)
    ...
    def test_stamp(self):
        ...
        self.log(f"Bắt đầu test trên {total_test_pages} trang...")

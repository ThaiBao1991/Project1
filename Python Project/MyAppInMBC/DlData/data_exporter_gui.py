...
class DataExporterGUI:
    ...
    def create_exporter_window(self):
        ...
        fetch_progress_frame = tk.LabelFrame(self.top_level, text="Tiến độ Lấy dữ liệu", padx=5, pady=5)
    ...
    def update_export_progress_labels(self, current_row, cell_b_data, non_empty_count):
        """Cập nhật các nhãn hiển thị tiến độ Xuất Data."""
        self.export_current_row_label.config(text=str(current_row))
        self.export_column_b_data_label.config(text=str(cell_b_data)[:50] + "..." if len(str(cell_b_data)) > 50 else str(cell_b_data))

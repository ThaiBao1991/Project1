...
def on_resize(event):
    width = root.winfo_width()  # Lấy chiều rộng hiện tại
    height = root.winfo_height()  # Lấy chiều cao hiện tại
    root.title(f"Gửi Dữ Liệu Khách Hàng - {width}x{height}")  # Cập nhật tiêu đề

# Hàm reset tiêu đề khi thả ra
def on_release(event):
    root.title("Gửi Dữ Liệu Khách Hàng")  # Trở về tiêu đề mặc định
...
def open_config(event=None):
    ...
    config_window.resizable(False, False)  # Không cho thay đổi kích thước

    # Frame chứa Label, Entry và Button
    frame_config = tk.Frame(config_window, bg="#e8ecef")
    frame_config.pack(pady=50)

    # Label "Địa chỉ lưu dữ liệu"
    label_save_path = tk.Label(frame_config, text="Địa chỉ lưu dữ liệu:", 
                              font=("Helvetica", 14, "bold"), bg="#e8ecef", fg="#2c3e50")
    label_save_path.grid(row=0, column=0, padx=20, pady=20, sticky="e")

    # Entry để nhập địa chỉ
    global entry_save_path  # Để sử dụng trong hàm save_config
    entry_save_path = tk.Entry(frame_config, width=40, font=("Helvetica", 12), bd=2, relief="sunken", bg="#ffffff")
    entry_save_path.grid(row=0, column=1, padx=20, pady=20)

    # Nút "Chọn nơi lưu dữ liệu"
    ...
    btn_choose_path.grid(row=0, column=2, padx=20, pady=20)
    ...
    # Nút "Close"
    btn_close = tk.Button(frame_buttons, text="Close", command=config_window.destroy, 
...
root.geometry("1200x600")  # Kích thước ban đầu
root.configure(bg="#e8ecef")

icon = tk.PhotoImage(file="LogoMabuchiWhite.png")
root.iconphoto(True, icon)

# Bắt sự kiện kéo thả và thả ra
root.bind("<Configure>", on_resize)  # Khi cửa sổ thay đổi kích thước
root.bind("<ButtonRelease-1>", on_release)  # Khi thả chuột trái

# Gán phím tắt Alt + Shift trái + S để mở cửa sổ Config
root.bind("<Alt-Shift-S>", open_config)

# Frame chứa Label tiêu đề "Gửi email khách hàng tự động" ở trên cùng
...
# Frame chứa Label, Entry, và Nút chọn file
frame_top = tk.Frame(root, bg="#e8ecef")
frame_top.pack(pady=50)

# Hàng 1: Chọn file Excel
...
# Hàng 2: Chọn file TXT dữ liệu tháng

...
def open_data_window(parent):
    ...
    def add_data():
        ...
        # Tính số cột dựa trên số lượng label
        num_cols = len(data_df.columns)
        max_rows = 10  # Tăng lên 10 dòng mỗi cột
        num_grid_cols = max(1, math.ceil(num_cols / max_rows))
        rows_per_col = math.ceil(num_cols / num_grid_cols)

        # Tạo canvas với thanh cuộn ngang và dọc
        canvas = tk.Canvas(add_window, bg="#e8ecef")
        scrollbar_y = ttk.Scrollbar(add_window, orient="vertical", command=canvas.yview)
        scrollbar_x = ttk.Scrollbar(add_window, orient="horizontal", command=canvas.xview)
        ...
        window_width = min(400 + 300 * (num_grid_cols - 1), 1200)

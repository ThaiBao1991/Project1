...
def chon_file_txt():
    """Chọn file TXT hoặc CSV và chuyển thành data_work.csv"""
    global entry_file, current_period, data_df
    
    # Mở hộp thoại chọn file với cả 2 loại TXT và CSV
    ...
    try:
        # Xử lý file CSV trực tiếp
        if file_path.lower().endswith('.csv'):
            # Kiểm tra encoding và đọc file CSV
            encodings = ['utf-8-sig', 'utf-8', 'latin1', 'iso-8859-1', 'utf-16']
            for encoding in encodings:
                try:
                    df = pd.read_csv(file_path, encoding=encoding)
                    if not df.empty:
                        # Lưu thẳng vào data_work.csv
                ...
                return
        # Xử lý file TXT như cũ
        else:
            convert_txt_to_csv(file_path)
        
        # Load lại dữ liệu sau khi có file mới
        from .data import initialize_data

...
### Mục tiêu & Bối cảnh
- Khi mạng bị chập chờn, timeout, hoặc ngắt kết nối tạm thời, trước đây code thực hiện `break` làm dừng toàn bộ 6,410 Days và UI báo "LỖI NGHIÊM TRỌNG".
- Nâng cấp hệ thống để phân loại chính xác sự cố mạng tạm thời, tự động thử lại tại chỗ, và không làm gián đoạn toàn bộ tiến trình.

### Chi tiết thay đổi
1. **`auto_ai_worker.py`**:
   - Thêm vòng lặp Auto-retry Lượt 1 cho từng Day (tối đa 3 lần thử, có khoảng nghỉ 10s để ổn định mạng).
   - Xóa bỏ các lệnh `break` nguy hiểm: Nếu sau 3 lần thử mạng vẫn lỗi, ghi nhận cảnh báo và chuyển tiếp tục sang Day sau (vẫn lưu lại tiến độ bài học đã tải), không dừng cả tiến trình.
2. **`gemini_safe.py`**:
   - Tách biệt rõ thông báo lỗi `ErrorKind.NETWORK`: `🌐 Lỗi kết nối mạng: ... Chờ Xs để kết nối lại...` với Exponential Backoff tự nhiên.
3. **`AskCpl.py`**:
   - Cải tiến khối `except` trong luồng worker: Nhận diện lỗi mạng (`requests.exceptions.ConnectionError`, `Timeout`, `OSError`) và thông báo thân thiện: `🌐 Tạm dừng do sự cố mạng: ... Bạn có thể bấm 'Bắt đầu' lại bất cứ lúc nào để tiếp tục.` (loại bỏ chữ "LỖI NGHIÊM TRỌNG").
4. **`exercise_server.py`**:
   - Bọc fallback `127.0.0.1` khi IP cấu hình máy chủ (`host`) không còn khả dụng trên máy (tránh crash khi đổi Wi-Fi / IP LAN).

### Kiểm thử & Trạng thái
- ✅ Syntax check (`py_compile`) cả 4 file: ALL SYNTAX OK.
- ✅ Unit test error classification & network retry logic OK.
- ✅ DONE.

---

## 2026-08-23 — Hoàn tất Đồng bộ Hóa Cooldown Key & Cập Nhật Bảng UI Real-time

### Mục tiêu & Bối cảnh
- Đồng bộ toàn diện trạng thái Cooldown Key từ Worker ngầm sang File `settings.json` và cập nhật trực tiếp lên bảng giao diện UI Treeview thời gian thực.
- Khắc phục lỗi tất cả key bị rơi vào cooldown khi chạy tải hàng loạt (do `lock_after_success=True`).

### Chi tiết thay đổi
1. **`AskCpl.py`**:
   - Gắn `self.refresh_api_keys_list = refresh_list` toàn cục.
   - Khi có bất kỳ cập nhật trạng thái key nào từ Worker $\rightarrow$ tự động render lại toàn bộ hàng trong Treeview ngay lập tức.
   - Tự động làm mới khi người dùng chuyển sang Tab Quản lý API Key (`<Visibility>` binding).
   - Thêm nút **"🔄 Làm mới"** và nút **"🔓 Xóa Cooldown"** trên thanh công cụ Quản lý Key.
2. **`auto_ai_worker.py`**:
   - Đặt `lock_after_success=False` trong `GeminiCoordinator`: Khi tải 6,410 Day hàng loạt, các account xoay vòng Round-Robin tự nhiên với nhịp an toàn (`3.5s - 5.0s` + Jitter). **Chỉ khóa 60 phút khi thực sự gặp lỗi 429** (không khóa sau khi gọi thành công).
3. **`gemini_safe.py`**:
   - `AccountPool`: Cập nhật `lock_account()` để gán `cooldown_until` cho tất cả key cùng email.
   - `_usable_keys()` & `account_locked()`: Đối soát cả RAM và dữ liệu file, tự động mở khóa khi hết hạn.
4. **`settings.json`**:
   - Đã reset toàn bộ 155 key về trạng thái `active` sẵn sàng tải lại ngay.

### Kiểm thử & Trạng thái
- ✅ Đã xóa sạch cooldown cũ trên 155 key trong `settings.json`.
- ✅ Syntax check (`py_compile`) cả 3 file: ALL SYNTAX OK.
- ✅ DONE.
...
## 2026-08-22 — Fix gemini_safe.py: QUOTA_RATE khóa ngay, không retry

### Thay đổi

- **`gemini_safe.py` dòng 433–440**: Sửa nhánh `ErrorKind.QUOTA_RATE` (429 RPM/TPM).
  - **Trước**: Retry tối đa `max_transient` (6) lần × 65s = lãng phí ~7 phút trên cùng key đã quá hạn.
  - **Sau**: Khi gặp 429 RPM/TPM lần đầu → `pool.lock_account()` (cooldown 60 phút) + `_mark("exhausted")` (ghi vào `settings.json` qua callback) + chuyển key ngay.
- **Lợi ích**: Key bị RATE limit hiện thị `exhausted` trong UI quản lý key (không còn "active" giả); không lãng phí thời gian chờ; an toàn hơn với Google anti-abuse.

### Trạng thái

- ✅ DONE. Syntax check OK. Áp dụng ngay lần chạy tiếp theo (không cần restart nếu `gemini_safe.py` được import fresh).
...
- Hộp thoại xác nhận hiển thị rõ số lượng: _"Bạn có chắc chắn muốn xóa N Key đã chọn?"_.
...
- **Bối cảnh:** Xử lý yêu cầu tạo lộ trình tiến hóa công nghệ tự chủ _"Thiên Công Khai Vật & Khôi phục văn minh từ thời kỳ đồ đá/chiến tranh đến chế tạo máy tính từ cát/silicon"_.
- **Vấn đề đã khắc phục:**
...
- **Giải pháp**:
  ...
  - **Memory Advanced (Phỏng vấn liên hoàn)**:
    ...
    - Bước 2: Gửi câu hỏi mớm để hỏi Tên Chủ Đề (vd: _"Chủ đề là gì chỉ mỗi chủ đề thôi"_). Cập nhật biến đếm số bài của chủ đề đó.

    - Bước 3: Nếu Chủ đề chưa hoàn thành đủ số lượng mục tiêu, Addon vòng lặp qua các "Chi tiết" được cài đặt trên giao diện (ví dụ: Chi tiết 1: "Từ vựng", Chi tiết 2: "Ngữ pháp"). Mỗi chi tiết sẽ kích hoạt 1 câu hỏi độc lập với AI (vd: _"Hỏi từ vựng là gì, chỉ ghi các từ vựng thôi"_).

    - Câu trả lời của AI cho các Chi tiết này sẽ được bóc tách và cộng dồn vào Object bộ nhớ cục bộ (`topicMemory`).

  - **Memory Injector**: Trước khi gửi `Day X+1`, Addon tự động format bộ nhớ `topicMemory` thành một khối text gọn gàng (vd: _Chủ đề: Con người 1/4 \n Từ vựng đã học: ... \n Ngữ pháp đã học: ..._). Khối text này được gắn trực tiếp vào cuối câu lệnh `Day X+1` để "mớm" trí nhớ cho AI. Các chủ đề đã đạt mục tiêu (ví dụ 4/4) sẽ được rút gọn thành chữ "Hoàn tất" để tiết kiệm token.
...
- **Giải pháp**:
  ...
  2. **Token-Optimized Prompt Injector (Nén Trí Nhớ)**:
     - Trong `content_script.js`, Add-on dò xem Ngày hiện tại thuộc Chương nào. Nếu đang ở giữa Chương, nó sẽ ghép thêm câu tóm tắt của các ngày trước _thuộc cùng Chương đó_.
  ...
  3. **Auto Summarizer**: Sau khi lấy xong bài học chính, Add-on tự động bắn thêm lệnh phụ _"Tóm tắt cực kỳ ngắn gọn kiến thức trên"_ và lưu vào `historySummaries`.
...
- **Giải pháp**:
  ...
  3. **Auto Summarizer**: Sau khi lấy xong bài học chính, Addon tự động bắn thêm lệnh phụ _"Tóm tắt cực kỳ ngắn gọn kiến thức trên"_ và lưu vào `historySummaries`.
...
oadmap\_[ten_profile] trong chrome.storage.local.
...
oadmap\_{profile} thay vì nhúng vào ddonConfigs.
...
- Mỗi Day giờ có dòng: _File goc: X KB (~Y trang) - da tai ve Desktop\Data_
...
- **Fix:** C?p nh?t logic quet file HTML trong AskCpl.py (tab Bai t?p) ?? h? tr? nh?n di?n c? ten file d?ng c? (day*1.html) va ??nh d?ng m?i xu?t t? Addon (001_Day 1...html). ?a ??i c? ch? parse t? startswith('day*') sang regex k?t h?p ki?m tra t? khoa linh ho?t, ??ng th?i s?p x?p (sort) l?i danh sach chu?n xac ?? tr?n l?n c? 2 chu?n file khong b? l?i.
...
- **Fix**: Sửa waitForResponseComplete trả về false khi hết thời gian chờ/kẹt mạng. Sửa hàm askSecondaryPrompt bắt kết quả false này và trả về **ASKCPL_TIMEOUT**. Tại \_runNextDayAttempt, nếu hỏi bồi hoặc tóm tắt trả về **ASKCPL_TIMEOUT** thì lập tức return 'retry' để hệ thống mở Chat mới và hỏi lại toàn bộ nội dung của Ngày hiện tại.
